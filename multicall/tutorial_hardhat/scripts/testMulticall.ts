...
import { Multicall, SimpleStorage } from "../typechain-types";
...
async function main() {
  ...
  const multicall = await Multicall.deploy() as unknown as Multicall;
  ...
  const decodedDataOwner = simpleStorage.interface.decodeFunctionResult("getValue", returnData[0]);
  const decodedDataAddr1 = simpleStorage.interface.decodeFunctionResult("getValue", returnData[1]);
  const decodedDataAddr2 = simpleStorage.interface.decodeFunctionResult("getValue", returnData[2]);
